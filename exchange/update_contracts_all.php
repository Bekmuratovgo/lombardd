...
$tinkoff_block = HL\HighloadBlockTable::getById(HI_TINKOFF_ID)->fetch();
$tinkoff_entity = HL\HighloadBlockTable::compileEntity($tinkoff_block);
$tinkoff_entityData = $tinkoff_entity->getDataClass();
...
foreach ($arFeedElements as $arFeedElement)
{
	...
	$tinfoff_rsData = $tinkoff_entityData::getList(array(
		'select' => array('ID'),
		'order' => array(),
		'filter' => array(
			"UF_CONTRACT_CODE" => $contractId,
	...
	while($arData = $tinfoff_rsData->fetch())
	{
		$tinkoff_entityData::update($arData['ID'], array(
			'UF_1C_COMPLETE' => 'Y'
		));
	}
